...
public class Path<T> implements Iterable<Path<T>.Segment> {
    ...
    /**
     * Creates a new, empty path containing a start location. Essentially this represents a path
     * from the start location to itself with a total cost of "0".
     *
    ...
    /**
     * @return The location at the end of this path, which may be the start location if this path
     * contains no segments (i.e. this path is from the start location to itself).
     */
    public T getEnd() {
        if(path.size() == 0) {
            return start;
     ...
     * the start location and itself, this iterator contains no elements. This iterator does not
    ...
    public class Segment {
         ...
         *
         * @param start The starting location of this segment.
