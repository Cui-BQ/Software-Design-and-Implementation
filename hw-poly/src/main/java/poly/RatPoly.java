...
public final class RatPoly {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        //{inv: All RatTerms in the lst who's index < i has a larger expt than newTerm.expt}
    ...
    public RatPoly add(RatPoly p) {
        ...
        for (int i = 0; i < p.terms.size(); i++){
            sortedInsert(ret, p.terms.get(i));
        }
        return new RatPoly(ret);
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        for (int j = 0; j < p.terms.size(); j++){
            for (int i = 0; i < this.terms.size(); i++){
                sortedInsert(r, p.terms.get(j).mul(this.terms.get(i)));
            }
        }
    ...
    public RatPoly div(RatPoly p) {
        ...
        List<RatTerm> r = new ArrayList<>();
        RatPoly c_t = new RatPoly();
        // term-by-term copy of all terms in this.terms to c_t
        for (int i = 0; i < terms.size(); i++){
            sortedInsert(c_t.terms, this.terms.get(i));
        }
        // { Inv: c_t = c_t - the term with lowest degree in r * p }
        while (c_t.degree() >= p.degree()){
            int c_t_degree = c_t.degree();
            sortedInsert(r, c_t.getTerm(c_t.degree()).div(p.getTerm(p.degree())));
            c_t = c_t.sub(p.mul(new RatPoly(c_t.getTerm(c_t.degree()).div(p.getTerm(p.degree())))));
            if (c_t_degree == c_t.degree()){
                break;
            }
        }
        return new RatPoly(r);
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        List<RatTerm> r = new ArrayList<>();
        for (RatTerm term : terms) {
            if (!term.antiDifferentiate().isZero()) {
                r.add(term.antiDifferentiate());
            }
        }
