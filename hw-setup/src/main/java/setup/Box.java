...
public class Box implements Iterable<Ball> {
    ...
    public Box(double maxVolume) {
        if (maxVolume < 0){
            throw new IllegalArgumentException("maxVolume < 0");
        }
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        // Your code goes here.  Remove the exception after you're done.
        Set<Ball> temp = new LinkedHashSet<>();
        for (Ball ball : this) {
            temp.add(ball);
        }
        List<Ball> sort = new ArrayList<>(temp);
        sort.sort(Comparator.comparingDouble(Ball::getVolume));
